...
describe('кольцо выбрано так, что пост может стать узким местом', () => {
  it('плечо взято из графа дорог и оно одно из самых коротких', () => {
    const lengths = Object.values(WORLD.world.edges)
      .map((edge) => edge.km)
      .sort((a, b) => a - b)

    /*
     * КОРОТКОЕ ПЛЕЧО — ОБЯЗАТЕЛЬНОЕ УСЛОВИЕ ПРОГОНА: чем меньше ход, тем
     * большую долю круга занимает перевалка и тем раньше упирается сеть.
     *
     * Раньше здесь стояло «самое короткое», и на карте округа это было одно и
     * то же: Тула — Калуга, 110 км. На карте страны появилось ребро короче
     * (Тольятти — Самара, 95 км через плотину Жигулёвской ГЭС), и требование
     * «ровно самое короткое» стало привязкой прогона к случайному факту чужого
     * файла: любое новое короткое ребро на другом конце страны роняло бы тест
     * про посты. Проверяется то, что нужно по существу — плечо из первой
     * десятки самых коротких на карте.
     */
    expect(LEG_KM).toBeGreaterThan(0)
    expect(LEG_KM).toBeLessThanOrEqual(lengths[Math.min(9, lengths.length - 1)])
  })
