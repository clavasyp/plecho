 ...
 *
 * ─── ПОДНИМАТЬ ЭТО ЧИСЛО НЕЛЬЗЯ, И ВОТ ЗАМЕР ───────────────────────────────
 *
 * Проверено подъёмом до десяти на прогоне в 240 суток. Соблазн был законный:
 * конкуренты малы, мир тает, и больше машин на линии выглядит как прямой путь
 * к тому, чтобы город наконец получил свою норму.
 *
 * Деньгам стало лучше: парк мира 17 → 22, годовая касса агрессивного
 * 2.78 → 3.75 млн, то есть +35%. А ПЕТЛЕ СТАЛО ХУЖЕ: эталонный тест роста
 * (tick.growth.test.ts) уехал с 28.82 суток на 34.35 и перестал укладываться в
 * свой предел.
 *
 * Причина в том, что решает НЕ РАЗМЕР ПАРКА, А ЕГО РАЗБИЕНИЕ. Машины одной
 * линии выходят из одной точки и выдерживают интервал (departureGapKm в
 * logistics/line.ts); на длинном перегоне очередь растягивается, и город
 * получает груз пачками вместо ровного потока. Счётчик снабжения рвётся не от
 * нехватки тонн — он рвётся от ПАУЗ между пачками: в разборе видно, что мука и
 * пиломатериалы лежат с запасом на сутки, а топливо в тот же тик равно нулю.
 * Больший потолок означает меньше линий на то же кольцо, то есть более крупные
 * пачки и более длинные паузы.
 *
 * Значит правильный рычаг — научить планировщика ДЕЛИТЬ кольцо на несколько
 * линий, а не поднимать этот потолок. Пока он не сделан, шесть остаются.
 */
...
function rankedPlans(
  ...
  // Что контора УЖЕ возит в города. Считается один раз на весь перебор: у
  // сортировки это самый горячий цикл, а набор за неё не меняется.
  const served = servedCargoes(company)

  /*
   * ДОВЕСТИ ГОРОД ДО КОНЦА ВАЖНЕЕ, ЧЕМ ВЗЯТЬ КОЛЬЦО ПОДОРОЖЕ — но только у тех,
   * у кого есть на это место.
   *
   * Замер, ради которого ключ и появился: за игровой год пять городов получили
   * хоть тонну, и КАЖДЫЙ ровно один вид груза. Городов с двумя грузами — ноль
   * из пятидесяти трёх, поэтому конъюнкция, от которой зависит рост, не
   * наступала ни разу за 35 040 тиков. Планировщик города не видел вовсе.
   *
   * ПЕРВЫЙ ЗАХОД БЫЛ СЛАБЫМ, И ЭТО ЗАМЕРЕНО. Ключ стоял тай-брейком внутри
   * десятипроцентной ступени по прибыли — и не изменил ни одного решения за
   * 240 суток: числа трёх контор совпали с прежними бит в бит. Причина простая:
   * кольца к уже обслуживаемому городу в такую узкую полосу просто не попадают,
   * мир слишком разрежен. Значит либо город — цель, либо его нет.
   *
   * ПОЧЕМУ НЕ У ВСЕХ. У осторожного потолок в одну линию: город для него
   * недостижим по построению, и ключ только испортил бы ему выбор
   * единственного кольца. Ключ включается там, где линий больше одной, то есть
   * там, где ВЫБОР МЕЖДУ ГОРОДАМИ вообще существует.
   */
  const chasesCities = MAX_LINES[personality] > 1

  const keyed = plans.map((plan) => ({
    plan,
    keys: [
      characterKey(plan, personality, playerCities, niche),
      ...(chasesCities ? [cityKey(plan, served)] : []),
      ...orderedByPersonality(
        personality,
        staffed(company, plan) ? 0 : 1,
        moneyBucket(plan, personality),
      ),
      // Точная прибыль — последней: внутри одной десятипроцентной ступени
      // выбор всё равно должен идти по деньгам.
...
/**
 * Шаг денежной лестницы: во сколько раз должны различаться кольца, чтобы
 * деньги решали.
 *
 * Десять процентов. Ниже этого разница в прибыли перестаёт быть содержательной:
 * она в пределах того, что даёт случайный водитель, пара километров маршрута
 * или качество покрытия на одном ребре. Внутри ступени решать должно что-то
 * другое — и решает город.
 */
const MONEY_STEP = 1.1

/**
 * Денежный ключ, огрублённый до десятипроцентных ступеней.
 *
 * ЗАЧЕМ ОГРУБЛЕНИЕ. Точная прибыль в рублях — число с плавающей запятой, и двух
 * одинаковых значений в мире из трёх с половиной тысяч планов не бывает. Значит
 * любой ключ, стоящий ПОСЛЕ точной прибыли, не срабатывает никогда: ничьих, для
 * разрыва которых он заведён, попросту не возникает. Ступень создаёт ничьи там,
 * где разница в деньгах и так ничего не значит.
 *
 * Логарифм, а не деление на постоянную: кольца различаются по прибыли на
 * порядки (от единиц рублей в час до двух сотен), и ступень должна быть
 * ОТНОСИТЕЛЬНОЙ. При постоянном шаге все дешёвые кольца слиплись бы в одну
 * ступень, а дорогие разъехались бы по своим.
 *
 * Неположительная прибыль кладётся в одну ступень на дне: убыточные кольца
 * между собой не ранжируются, и порядок среди них задаёт следующий ключ.
 */
function moneyBucket(
  plan: ScriptedPlan,
  personality: CompetitorPersonality,
): number {
  const value = -moneyKey(plan, personality)
  if (!(value > 0)) return Number.MAX_SAFE_INTEGER
  return -Math.floor(Math.log(value) / Math.log(MONEY_STEP))
}

/**
 * Какие потребительские грузы контора УЖЕ привозит в каждый город.
 *
 * Считается по её линиям, а не по машинам: линия — это замысел, и он остаётся
 * замыслом, даже когда машина в ремонте. Ключ карты — город, значение — набор
 * грузов.
 */
function servedCargoes(company: Company): Map<CityId, Set<CargoType>> {
  const served = new Map<CityId, Set<CargoType>>()

  for (const line of Object.values(company.lines)) {
    for (const stop of line.stops) {
      for (const cargo of stop.unload) {
        if (!CONSUMER_CARGO.includes(cargo)) continue
        const at = served.get(stop.nodeId) ?? new Set<CargoType>()
        at.add(cargo)
        served.set(stop.nodeId, at)
      }
    }
  }
...
/**
 * ГОРОД КАК ЦЕЛЬ, А НЕ КАК ТОЧКА СБЫТА.
 *
 * Главная петля игры устроена так, что город растёт, только когда закрыты ВСЕ
 * ТРИ потребительских груза разом (economy/consumption.ts). А планировщик
 * города не видел вовсе: он ранжировал кольца по деньгам и сдавал продукцию
 * туда, где выгоднее. Замер за игровой год: пять городов получили хоть тонну, и
 * КАЖДЫЙ — ровно один вид груза. Городов с двумя грузами — ноль из пятидесяти
 * трёх, поэтому конъюнкция не наступала ни разу за 35 040 тиков.
 *
 * Ключ отвечает на один вопрос: привезёт ли это кольцо ВТОРОЙ груз туда, куда
 * контора уже возит первый. Если да — оно лучше при прочих равных.
 *
 * ПОЧЕМУ ЭТО ТАЙ-БРЕЙК, А НЕ СЛАГАЕМОЕ К ПРИБЫЛИ. Надбавка, не
 * пропорциональная тонно-километрам, — ровно тот класс правки, который уже
 * ломал баланс этого проекта: разбор в шапке CARGO_PREMIUM (data/recipes.ts),
 * где ценность груза перебивала штраф за порожний пробег и метрика, объявленная
 * главной, переставала предсказывать прибыль. Здесь деньги остаются главными;
 * город решает только внутри десятипроцентной ступени, где деньги уже ничего не
 * значат.
 *
 * Ключ отрицательный не нужен: меньше — лучше, а «уже возим туда» это 0.
 */
function cityKey(
  plan: ScriptedPlan,
  served: ReadonlyMap<CityId, ReadonlySet<CargoType>>,
): number {
  for (const stop of plan.stops) {
    const already = served.get(stop.nodeId)
    if (already === undefined) continue
    for (const cargo of stop.unload) {
      // Именно ДРУГОЙ груз: везти в город второй машиной то же самое значит
      // удваивать уже закрытое, а конъюнкции это не двигает.
      if (CONSUMER_CARGO.includes(cargo) && !already.has(cargo)) return 0
    }
  }
  return 1
}
