...
describe('граф дорог', () => {
  ...
  it('километры положительные и правдоподобные для ЦФО', () => {
    // Нижняя граница отсекает опечатку вроде 18 вместо 180, верхняя — лишний
    // ноль: округ такого размера, что 800 км это уже поездка сквозь него.
    for (const edge of EDGES) {
      expect(edge.km, edge.id).toBeGreaterThanOrEqual(30)
      expect(edge.km, edge.id).toBeLessThanOrEqual(800)
    }
  })
