 ...
 * Размер карты теней.
 *
 * Число приезжает из render/quality.ts: на машине без видеокарты сторона вдвое
 * меньше. Причина в том, что тень — второй по величине потребитель пикселей
 * после постпроцессинга: 2048 на сторону это проход глубины в 4.19 мегапикселя,
 * то есть ВДВОЕ больше самого кадра на такой машине. Разбор цены — там же.
 */
const SHADOW_MAP = shadowMapSide
...
export function Scene(): JSX.Element {
  ...
  useLayoutEffect(() => {
    ...
    /*
     * ТИП КАРТЫ ТЕНЕЙ — PCF, А НЕ PCFSoft, И ЭТО НЕ ПОНИЖЕНИЕ КАЧЕСТВА.
     *
     * Здесь стояло PCFSoftShadowMap. В three 0.185 он ОБЪЯВЛЕН УСТАРЕВШИМ и
     * молча подменяется на PCFShadowMap — с предупреждением в консоли и без
     * всякого эффекта. То есть мягких теней у игры не было и раньше; была
     * только строка кода, обещавшая их, и жёлтая строка в консоли у каждого,
     * кто открыл игру. Пишем то, что происходит на самом деле.
     */
    ...
    const report = (): void => {
      ...
      console.info(
        `[ПЛЕЧО] ${card.видеокарта} · буфер ${card.буфер} (${card.мегапикселей} Мпикс) · ` +
          ...
          (soft || softwareRendering
            ? '\n[ПЛЕЧО] Видеокарта браузеру недоступна — кадр считает процессор. ' +
              'Игра сама перешла на облегчённый режим: меньше пикселей, ' +
              'вчетверо меньше карта теней, без сглаживания. ' +
              'Полную картинку вернёт аппаратное ускорение — посмотрите chrome://gpu.'
            : ''),
      )
    }
      ...
      */}
      <EffectComposer
        /*
          ...
          На машине БЕЗ ВИДЕОКАРТЫ здесь ноль (render/quality.ts). Разница
          принципиальная: на железе четырёхкратное сглаживание заложено в
          растеризатор и почти бесплатно, на процессоре это буквально вчетверо
          больше работы на каждую грань плюс полноэкранный резолв каждый кадр.
          Цена замерена: контраст ленты дороги на предельном отдалении падает с
          0.081 до 0.061. Порог видимости при этом не пробивается — ленту держит
          пиксельный пол шейдера, а не сглаживание.
        */
      ...
      >
        <Bloom
          ...
          mipmapBlur
          /*
            Три последних уровня пирамиды (20×12, 10×6, 5×3 пикселя) дают шесть
            проходов ради 0.04% закраски кадра. На видеокарте это незаметно в
            обе стороны, на процессоре шесть смен цели рендера — уже цена.
            Разница в картинке на ночной Москве, где светятся только окна:
            0.219 из 255 при собственном шуме прибора 0.231, то есть
            неразличимо.
          */
          levels={bloomLevels}
        />
