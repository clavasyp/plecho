...
describe('сезоны на карте', () => {
  ...
  /*
   * ЗДЕСЬ СТОЯЛО «зимой земля белеет ВДВОЕ», и это перестало быть правдой не по
   * ошибке, а по решению. История важнее самой проверки, поэтому она записана.
   *
   * Прежде три тёмных сезона были почти чёрными (#212b32, #262a22, #212528), и
   * зима действительно светлела вдвое. Ровно из этого размаха и вырос дефект,
   * который стоил карте дорожной сети: земля ходила по линейной яркости от
   * 0.022 до 0.099, а федеральное полотно стоит на 0.0995 — и тон дороги
   * обязан был где-то пересечь тон земли. Замер понедельно: контраст
   * 0.005–0.015 на 42–56-е сутки при уровне плёночного зерна 0.012, то есть
   * два месяца в году сети на карте не было вовсе (разбор — в шапке
   * seasonGround в palette.ts).
   *
   * Лечение подняло три тёмных сезона примерно вдвое, и сезоны стали
   * различаться ТОНОМ, а не яркостью. Значит и проверять надо тон: зима —
   * единственный холодный и синий, лето — тёплое и хаки. Яркость зимы при этом
   * осталась НАИБОЛЬШЕЙ из четырёх, и это утверждение здесь тоже есть — просто
   * без множителя, которого больше нет.
   */
  it('зима — самая светлая и самая синяя земля года', () => {
    expect(january.winter).toBeGreaterThan(0.9)

    // Зима по-прежнему светлее любого другого сезона, хотя и не вдвое.
    expect(luma(january.ground)).toBeGreaterThan(luma(july.ground))
    expect(luma(january.ground)).toBeGreaterThan(luma(october.ground))

    // Синева — то, чем зима теперь отличается в первую очередь: у снега синий
    // канал выше зелёного, у летнего хаки наоборот.
    expect(january.ground.b).toBeGreaterThan(january.ground.g)
    expect(july.ground.b).toBeLessThan(july.ground.g)
  })
