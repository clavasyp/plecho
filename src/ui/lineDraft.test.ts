...
describe('что в городе можно грузить и выгружать', () => {
  ...
  it('отмеченный, но невозможный здесь груз остаётся в списке', () => {
    const game = state()
    // Мука в Смоленске: мельницы там нет, но игрок её отметил — например,
    // переставив остановку в другой город. Молча вычистить чужое решение
    // нельзя, спрятать неотличимо от рабочего — тем более.
    const slots = loadSlots(
      game.world.industries,
      stop(SMOLENSK, [], [MILL.output]),
    )
