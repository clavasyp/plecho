 ...
 *
 * Идентификатор собирает фаза команд из НАЗВАНИЯ КОМПАНИИ, класса и номера:
 * 'player-zil-130-1'. Схема менялась дважды: 'zil-2' до среза 4, когда классов
 * не было, и 'zil-130-1' до среза 6, когда покупка ушла в общий applyCommand и
 * компанию стало нужно различать. Стартовая машина при этом выдаётся не
...
test('линия держит машину гружёной, а разовые рейсы — нет', async ({
  ...
  await page.evaluate(([from, to]) => {
    ...
    const drive = () => {
      const vehicle = store.getState().state.vehicles['player-zil-130-1']
