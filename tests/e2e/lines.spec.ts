...
/**
 * Кольцо, которое строит тест: Владимир → Рязань → Москва → Владимир.
 *
 * Маршрут выбран не за красоту, а за то, что он ЗАМКНУТ ПО ГРУЗУ и устойчив на
 * всю длину прогона:
 *
 *   Владимир — лесозаготовка, источник кругляка: он там есть всегда.
 *   Рязань   — ЦБК: принимает кругляк и отдаёт пиломатериалы, то есть даёт
 *              обратную загрузку из того же самого, что мы привезли.
 *   Москва   — единственный в игре потребитель, чей склад невозможно завалить
 *              одним ЗИЛом: она съедает пиломатериалов больше, чем машина
 *              успевает подвозить, и кольцо не встанет от переполнения.
 *
 * Гружёными оказываются два плеча из трёх — это и есть проектное решение,
 * которое проверяется. Третье, Москва → Владимир, порожнее: вывезти из Москвы
 * нечего, там нет ни одного предприятия. Идеального кольца в этом мире не
 * существует, и тест не притворяется, будто оно есть.
 */
const RING = [
  { nodeId: 'vladimir', unload: [], load: ['кругляк'] },
  { nodeId: 'ryazan', unload: ['кругляк'], load: ['пиломатериалы'] },
  { nodeId: 'moscow', unload: ['пиломатериалы'], load: [] },
] as const

const LINE_NAME = 'Лес на Москву'
...
test('линия держит машину гружёной, а разовые рейсы — нет', async ({
  ...
  const bought = await page.evaluate(
    (id) =>
      ...
      ).__plecho.getState().state.vehicles[id] !== undefined,
    CONTROL_TRUCK,
  )
  ...
  await page.evaluate(([from, to]) => {
    ...
    const drive = () => {
      const vehicle = store.getState().state.vehicles['zil-2']
